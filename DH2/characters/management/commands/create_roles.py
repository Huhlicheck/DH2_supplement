...
class Command(BaseCommand):
        ...
        roles_data = [
    {
        ...
        "role_bonus": "Sure Kill: In addition to the normal uses of Fate points, when an Assassin successfully hits with an attack, he may spend a Fate point to inflict additional damage equal to his degrees of success on the attack roll on the first hit the attack inflicts."
    },
    {
        "name": "Chirurgeon",
    ...
    },
    {
        ...
        for role_data in roles_data:
            role, created = Role.objects.get_or_create(
                name=role_data["name"],
                role_bonus=role_data["role_bonus"]
            )

            if created:
                # Add the required aptitudes to the role
                for aptitude_name in role_data["aptitudes"]:
                    aptitude = Aptitude.objects.filter(name=aptitude_name).first()
                    if aptitude:
